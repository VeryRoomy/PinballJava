//Name:              Date:
import java.awt.*;
public class DiagBumperRight
{
   public double[] xs = new double[4];
   public double[] ys = new double[4];
   public double myXSize, myYSize, myAngle;

   public DiagBumperRight(double x1, double y1, double s, double y, double a)
   {
      this.xs[0] = x1;
      this.ys[0] = y1;
      myXSize = s;
      ...
      xs[2] =  x1 + (myYSize*Math.sin(myAngle)) - (myXSize*Math.cos(myAngle)); //bottom right x
      ys[2] =  y1 + (myYSize*Math.cos(myAngle)) + (myXSize*Math.sin(myAngle));
   ...
   public void setAngle(double n)
   {
      ...
      ys[2] =  ys[0] + (myYSize*Math.cos(myAngle)) + (myXSize*Math.sin(myAngle));
   ...
   public boolean inBumper(Polkadot dot ,Graphics buff)
   {
      ...
      /*for(double x = 0; x <= myXSize*Math.cos(myAngle) ; x+=1.0)
      {
         ...
         buff.drawOval((int)(xs[0]-x), (int)(ys[0] + ((x*Math.tan(myAngle))*(-1 * sign(Math.sin(myAngle))))), 1, 1); 
         if(distance(xs[0]-x,ys[0] + (x*Math.tan(myAngle))*(-1 * sign(Math.sin(myAngle))), dot.getX(), dot.getY()) <= dot.getRadius() )
            return true;            
      }*/
      ...
      for(double x = 0; x <= myXSize*Math.cos(myAngle) ; x+=1.0)
      {
         double y = x * Math.tan(myAngle);
         buff.setColor(Color.BLUE);
         buff.drawOval((int)(xs[3]-x), (int)(ys[3] + (x*Math.tan(myAngle))*(-1 * sign(Math.sin(myAngle)))), 1, 1); 
         if(distance(xs[3]-x, ys[3] + (x*Math.tan(myAngle))*(-1 * sign(Math.sin(myAngle))),dot.getX(), dot.getY()) <= dot.getRadius() )
            return true;  
      } 
            // top left edge
      for(double k = 0; k <= Math.abs(myYSize*Math.sin(myAngle));k+=1.0)
      {
         double x = xs[1] + sign(k);
         double y = ys[1] + sign(x/Math.tan(myAngle));
         buff.setColor(Color.blue);
      ...
      // top right edge
         for(double k = 0; k <= Math.abs(myYSize*Math.sin(myAngle));k+=1.0)
         {
            double x = xs[0] + sign(k);
            double y = ys[0] + sign(x/Math.tan(myAngle));
            buff.setColor(Color.blue);
